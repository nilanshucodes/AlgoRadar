...
class ContactMessage(db.Model):
    ...
    created_at = db.Column(
        db.DateTime, nullable=False, default=lambda: datetime.now(IST)
    )
...
def index():
    ...
    for c in contests:
        resource = (
            c["resource"].lower()
            if isinstance(c["resource"], str)
            else str(c["resource"]).lower()
        )
        ...
        if time_filter:
            ...
            if time_filter == "today" and start_ist.date() != now.date():
                ...
                start_ist.date() > (now + timedelta(days=7)).date()
                or start_ist.date() < now.date()
            ):
                continue
            elif time_filter == "month" and (
                start_ist.date() > (now + timedelta(days=30)).date()
                or start_ist.date() < now.date()
            ):
                continue
    ...
    filtered.sort(
        key=lambda x: datetime.strptime(
            x["start_date"] + " " + x["start_time"], "%d-%m-%Y %H:%M"
        )
    )
